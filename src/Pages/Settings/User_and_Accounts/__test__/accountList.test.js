...
import AccountsUsersList from '../index'
...
describe("UserAndAccounts Page Component", () => {

    it('render Home page without error', async () => {
        const { rerender, getByTestId, asFragment } = render(<Provider store={store}><Router history={historyMock}><AccountsUsersList {...defaultprops} /></Router></Provider>);
        const listNode = await waitFor(() => getByTestId('user-accounts-page'));
        expect(listNode.children).toHaveLength(4);
        rerender(<Provider store={store}><AccountsUsersList {...defaultprops} /></Provider>);
