  ...
  return (
    ...
    />
      : <img
        ...
        loading="lazy"
        className={(props.url && props.url != null && !defaultImage) ? '' : props.className != undefined ? props.className : 'No-image'}
        alt={props.url}
      />
  );
};
...
function ImageIsValid(props) {
  const [image, setImage] = useState(false);
  
  useEffect(() => {
    if(props && props !== null && props !== "") {
      fetch(constants.API.COUNTRY.GET_FLAG_DOWNLOAD + props)
        .then(res => {
          if (res.status == 200) {
            setImage(true)
          } else {
