...
import ListedFeature from '../ListedFeatures'


const setup = (defaultProps) => {
    return shallow(<ListedFeature {...defaultProps} />);
}
...
describe("Header Component",()=>{
    ...
    feat_data_set:[1, 2, 5, 6, 7, 8, 9, 10, 13, 15, 14, 22, 23, 24, 25, 26, 27, 28, 29, 17, 18, 19, 16, 21, 20, 30, 31, 32, 33, 34, 35, 36, 37, 38, 3, 4, 42, 41, 12, 47, 11, 39, 40]
    }
    beforeEach(() => {
    ...
    test('Render Header Component  Without Error',()=>{
        const listed=findByTestAttr(wrapper,'listed')
        expect(listed.length).toBe(1)
        const name=findByTestAttr(wrapper,'name')
        expect(name.first().simulate('click').length).toBe(1)
       let wrapper1=setup({})
       const listed1=findByTestAttr(wrapper1,'listed')
